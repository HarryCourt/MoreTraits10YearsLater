        ...
        if ZombRand(0, 1001) <= chance then
            ...
            for i = 0, itterations do
                ...
                elseif roll <= 40 then
                    ...
                    inv:addItemOnServer(inv:AddItem(randomitem[ZombRand(tablelength(randomitem)) + 1]));
                elseif roll <= 60 then
                    local randomitem = {"Base.Bandage", "Base.BandageDirty", "Base.Pills", "Base.AlcoholWipes"};
                    inv:addItemOnServer(inv:AddItem(randomitem[ZombRand(tablelength(randomitem)) + 1]));
                elseif roll <= 80 then
                    local randomitem = { "Base.Plank", "Base.ChairLeg", "Base.MetalPipe", "Base.PickAxeHandle", "Base.TableLeg"};
                    inv:addItemOnServer(inv:AddItem(randomitem[ZombRand(tablelength(randomitem)) + 1]));
                elseif roll <= 95 then
                    local randomitem = { "Base.PipeWrench", "Base.LeadPipe", "Base.Golfclub", "Base.BaseballBat", "Base.Poolcue", "Base.TennisRacket"};
                    inv:addItemOnServer(inv:AddItem(randomitem[ZombRand(tablelength(randomitem)) + 1]));
                end
            end
